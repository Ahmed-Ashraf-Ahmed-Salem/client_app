...
/*    AND DECODE(NVL(LM_CLIENT.BRANCH_cODE, 'a'), 'a', 
    ...
    AND (
            (:clientType = 'cl'  AND EXISTS (
                SELECT 1 FROM lm_client c WHERE c.nationalno = DM_CLIENT_REGISTER.NATIONAL_ID
            ))
        OR (:clientType = 'pcl' AND NOT EXISTS (
                SELECT 1 FROM lm_client c WHERE c.nationalno = DM_CLIENT_REGISTER.NATIONAL_ID
            ))
        OR (:clientType = 'all')
        )         
...
oci_bind_by_name($stmt3, ":clientType", $clientType);
...
oci_bind_by_name($stmt4, ":clientType", $clientType);
