...
public interface DMClientAppRepository extends JpaRepository<DMClientApp, DMClientAppId> {
    ...
    @Query(value = """
        ...
        FROM 
            dm_clientapp dm,
            (SELECT lm_allemp.govern_code, lm_allemp.branch_code, lm_allemp.CODE, lm_allemp.NAMEA EMP_NAME, s.namea EMP_JOB, dep.NAMEA EMP_DEP, lm_allemp.EMPSTAT_CODE EMPSTAT_CODE
        ...
        WHERE 
            ...
            AND dm.app_flag = 'C'
            AND nvl(emp.EMPSTAT_CODE,'000') not in ('003')
            AND CASE 
