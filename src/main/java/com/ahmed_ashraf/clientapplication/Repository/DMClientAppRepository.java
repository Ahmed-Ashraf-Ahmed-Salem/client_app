...
@Repository
public interface DMClientAppRepository extends JpaRepository<DMClientApp, Long> {

    @Query(value = """
        SELECT dm.serial, dm.NATIONALNO
        FROM 
            dm_clientapp dm,
            (SELECT lm_allemp.govern_code, lm_allemp.branch_code, lm_allemp.CODE, lm_allemp.NAMEA EMP_NAME, s.namea EMP_JOB, dep.NAMEA EMP_DEP 
            ...
            lm_applic app
        WHERE 
            dm.EMP_BRANCH_CODE = emp.branch_code (+)
            AND dm.EMP_CODE = emp.CODE (+)
